...
| `run.start` / `run.status` / `run.result` / `run.replay` / `run.evidence` / `run.exportVerified` / `run.cancel` | Start a verified-change run; inspect durable state, export sealed exact evidence, or cancel the run tree        |
