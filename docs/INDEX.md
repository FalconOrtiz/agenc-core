...
| [ci-required-gates.md](ci-required-gates.md) | Fast `test:fast` classification, exact-SHA release gates, and the inactive optional GitHub App/ruleset design |
...
| [sdk.md](sdk.md) | Embed via `@tetsuo-ai/agenc-sdk` (socket + subprocess), including `startRun` model/provider and the generated transcript.v2 protocol slice |
...
| [reference/daemon.md](reference/daemon.md) | Daemon lifecycle, socket auth, deferred first messages, bypass consent, bounded-stop and prompt-hook-block survival, admission step identity, in-turn checkpoint resume gates, compaction recovery fields, and rebuilt transcript `turnResults` |
...
| [design/durable-runs-effects-events.md](design/durable-runs-effects-events.md) | M4 canonical run journal, honest effects, resume with pending reviews, in-turn checkpoint resume, terminal results, replay-safe cursors, crash matrix, and rollback |
