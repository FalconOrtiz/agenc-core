...
**Current release: 0.9.2.** The public one-line installer and npm package use
...
```bash
curl -fsSL https://get.agenc.ag/install.sh | sh -s -- --version 0.9.2
```
...
`--pin 0.9.2` targets a specific release and is the only update mode allowed to
...
native module ABI and Node-API version. Node 25 is now end-of-life; 0.9.2 is a
...
runtime **0.9.2** when you need parity with this doc set.
