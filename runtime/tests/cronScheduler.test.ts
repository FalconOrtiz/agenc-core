...
const TEST_ACTIVATION = {
  queueOwner: {
    kind: "session",
    conversationId: "cron-test-conversation",
  },
  workspaceRoot: "/cron-test-workspace",
} as const;
...
vi.mock("src/utils/messageQueueManager.js", async (importOriginal) => ({
  ...(await importOriginal<
    typeof import("src/utils/messageQueueManager.js")
  >()),
...
function task(
  overrides: Partial<CronTask> & { id: string; cron: string },
): CronTask {
  return {
...
describe("CronScheduler", () => {
  ...
  test("a session-only task is runnable only by its creating conversation", async () => {
    ...
    sched.stop(TEST_ACTIVATION);
  });

  test("a durable task binds to the frozen scheduler activation", async () => {
    const clock = new FakeClock(10 * 60_000);
    const enqueue = vi.fn();
    ...
    const sched = makeScheduler(clock, [durableTask], enqueue);
    ...
    expect(enqueue).toHaveBeenCalledTimes(1);
    expect(enqueue.mock.calls[0]?.[0].queueOwner).toEqual(
      TEST_ACTIVATION.queueOwner,
    );
    sched.stop(TEST_ACTIVATION);
  });

  test("a distinct concurrent activation is rejected and cannot stop the owner", async () => {
    const clock = new FakeClock(10 * 60_000);
    const enqueue = vi.fn();
    const sched = makeScheduler(
      clock,
      [task({ id: "owner003", cron: "* * * * *" })],
      enqueue,
    );
    const siblingActivation = {
      queueOwner: {
        kind: "session",
        conversationId: "sibling-conversation",
      },
      workspaceRoot: "/sibling-workspace",
    } as const;

    sched.start(TEST_ACTIVATION);
    expect(() => sched.start(siblingActivation)).toThrow(
      /different conversation or workspace/,
    );
    // A stale cleanup from the rejected sibling cannot stop the real owner.
    sched.stop(siblingActivation);
    await flush();
    await advanceAndFlush(clock, 1_000);
    expect(enqueue).toHaveBeenCalledTimes(1);
    sched.stop(TEST_ACTIVATION);
  ...
  test("a tick whose task load finishes after an activation switch is inert", async () => {
    const clock = new FakeClock(10 * 60_000);
    const enqueue = vi.fn();
    let capturedWake: (() => void) | undefined;
    let releaseOldDispatch!: (tasks: CronTask[]) => void;
    const oldDispatch = new Promise<CronTask[]>((resolve) => {
      releaseOldDispatch = resolve;
    });
    let loadCount = 0;
    const due = task({
      id: "owner004",
    ...
    const sched = new CronScheduler(
      {
        ...
        loadTasks: async () => {
          loadCount += 1;
          if (loadCount === 1) return [due];
          if (loadCount === 2) return oldDispatch;
          return [];
        },
        enqueue,
      },
    ...
    sched.start(TEST_ACTIVATION);
    await flush();
    expect(capturedWake).toBeTypeOf("function");
    capturedWake!();
    await flush();
    expect(loadCount).toBe(2);

    sched.stop(TEST_ACTIVATION);
    sched.start(replacementActivation);
    await flush();
    releaseOldDispatch([due]);
    await sched.drain();

    expect(enqueue).not.toHaveBeenCalled();
    expect(sched.getLastTelemetry()?.dispatched ?? 0).toBe(0);
    sched.stop(replacementActivation);
  });
});
