...
function renderPrePrivateNodeWrapperFixture(values: {
  readonly kind: "posix" | "cmd";
  ...
  if (values.kind === "cmd") {
    ...
    return [
      ...
      `if not defined AGENC_HOME set "AGENC_HOME=${batch(values.agencHome)}"`,
      `"${batch(values.nodeBin)}" "${batch(values.runtimeBin)}" %*`,
      "",
    ].join("\r\n");
  }
...
describe.skipIf(process.platform === "win32")("install.sh", () => {
  ...
  test("cached retry replaces an exact pre-private-Node metadata wrapper", () => {
    const home = join(work, "legacy-wrapper-home");
    mkdirSync(home, { recursive: true });
    const artifact = makeSyntheticArtifact(work);
    const manifest = writeManifest(work, artifact);
    const first = runInstaller({ home, manifest, args: ["--no-daemon"] });
    expect(first.status, first.stderr).toBe(0);
    const installed = paths(home, artifact.sha);
    const legacy = renderPrePrivateNodeWrapperFixture({
      kind: "posix",
      nodeBin: join(home, ".nvm", "versions", "node", "v25.9.0", "bin", "node"),
      runtimeBin: join(
        home,
        ".agenc",
        "runtime",
    ...
    writeFileSync(installed.wrapper, legacy, { mode: 0o755 });
    chmodSync(installed.wrapper, 0o755);
    rmSync(artifact.tarball);

    const retry = runInstaller({ home, manifest, args: ["--no-daemon"] });

    expect(retry.status, retry.stderr).toBe(0);
    expect(retry.stderr).toContain("already installed");
    expect(retry.stderr).not.toContain(
      "refusing to replace a wrapper not generated by AgenC",
    );
    expect(readFileSync(installed.wrapper, "utf8")).not.toBe(legacy);
...
function registerInstallPs1Tests(): void {
  ...
  describe("install.ps1 behavior", () => {
    ...
    test("PowerShell cached retry replaces an exact pre-private-Node metadata wrapper", () => {
      ...
      const legacy = renderPrePrivateNodeWrapperFixture({
        kind: "cmd",
        nodeBin: join(home, "AppData", "Node 25", "node.exe"),
        runtimeBin: join(
          home,
          ".agenc",
          "runtime",
          "0.10.0",
          "win-x64-native-node-abi-141",
          BIN_REL,
        ),
        agencHome: join(home, ".agenc"),
      });
      writeFileSync(installed.wrapper, legacy, { mode: 0o644 });
      chmodSync(installed.wrapper, 0o644);
      rmSync(artifact.tarball);

      const retry = runPowerShell(home, manifest);
      const output = `${retry.stdout}\n${retry.stderr}`;

      expect(retry.status, output).toBe(0);
      expect(retry.stdout).toContain("already installed");
