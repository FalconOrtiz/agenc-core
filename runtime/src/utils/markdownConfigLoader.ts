 ...
 * stopping there makes the parent project's `.agenc/` unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.agenc/worktrees/`) stay on the old behavior: their `.git`
...
export function getProjectDirsUpToHome(
  ...
  while (true) {
    ...
    const agencSubdir = join(current, '.agenc', subdir)
    ...
    try {
      statSync(agencSubdir)
      dirs.push(agencSubdir)
    } catch (e: unknown) {
...
async function loadMarkdownFilesForSubdirUncached(
  ...
  authority: CanonicalSettingsAuthority,
    ...
    //
    // Only add the main repo's copy when the worktree root's .agenc/<subdir>
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      ...
      if (!worktreeHasSubdir) {
        const mainAgenCSubdir = join(canonicalRoot, '.agenc', subdir)
        if (!projectDirs.includes(mainAgenCSubdir)) {
          projectDirs.push(mainAgenCSubdir)
        }
      }
    }
    ...
    // This prevents the same file from appearing multiple times when ~/.agenc is
...
> {
  ...
  const useNative = isEnvTruthy(process.env.AGENC_USE_NATIVE_FILE_SEARCH)
