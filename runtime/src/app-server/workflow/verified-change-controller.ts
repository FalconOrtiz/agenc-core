...
function buildImplementPrompt(ctx: RunContext, attempt: number): string {
  const lines = [
    ...
    "The runtime hard-stops this stage after 8 model turns. Use at most 8 tool calls and the fewest needed for the bounded change.",
    ...
    "Make the first source edit by tool call 5 at the latest; edit sooner when the targeted files are already clear.",
...
function buildVerifyAgentPrompt(
  ...
  return [
    ...
    "The runtime hard-stops this stage after 4 model turns. Use at most 4 tool calls: inspect the relevant diff, re-run at most one relevant command, and run at most one focused adversarial probe.",
