...
export const RUNTIME_SETTINGS_MINIMAL_EFFORT_SCHEMA_VERSION = 31;

/** Allow Meta Muse's verified `minimal` reasoning tier in durable settings. */
export const runtimeSettingsMinimalEffortMigration: SqlMigration = {
  version: RUNTIME_SETTINGS_MINIMAL_EFFORT_SCHEMA_VERSION,
  name: "runtime_settings_minimal_effort",
  apply(db) {
    const table = db
      .prepare(
    ...
    db.exec(`
      ...
      RENAME TO run_runtime_settings_without_minimal_effort;
      ...
      SELECT
        ...
        model, provider, profile, reasoning_effort, model_verbosity,
        service_tier, hooks_disabled
      FROM run_runtime_settings_without_minimal_effort;

      DROP TABLE run_runtime_settings_without_minimal_effort;
